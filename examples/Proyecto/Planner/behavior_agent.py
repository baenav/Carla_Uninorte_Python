...
from local_planner_behavior import LocalPlanner
...
class BehaviorAgent(Agent):
    ...
    def set_waypoints(self, waypoints_file, clean=False):
        """
        Inicializa la lista de waypoints a partir del archivo.

            :param waypoints_file: nombre del archivo que contiene los waypoints
        ...
        self._local_planner.set_global_plan(waypoints)
 ...
 """
        ...
        if self._local_planner.avoid_counter == 5 and self._local_planner.prev_state == 1:
 ...
 """
    ...
    def run_stop_step(self, debug=False):

        control = self._local_planner.run_stop_step(
            target_speed= min(self.behavior.max_speed, self.speed_limit - self.behavior.speed_lim_dist), debug=debug)
